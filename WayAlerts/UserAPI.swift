...
class UserAPI {
    ...
    var appDelegate: AppDelegate;//   = NSUserDefaults.standardUserDefaults().objectForKey(Constants.NSDKeys.appDelegage) as! AppDelegate
...
//    }
    ...
    func getAllUsers(sortedByDate:Bool = true, sortAscending:Bool = true) -> Array<User> {
        ...
        do {
            fetchedResults = try  self.managedObjectContext.executeFetchRequest(fetchRequest) as! [User]
    ...
    func updateUser(userItemToUpdate: User, newUserItemDetails: Dictionary<String, AnyObject>){
        ...
        let minionManagedObjectContextWorker:NSManagedObjectContext =
            NSManagedObjectContext.init(concurrencyType: NSManagedObjectContextConcurrencyType.PrivateQueueConcurrencyType)
        minionManagedObjectContextWorker.parentContext = self.managedObjectContext
    ...
    func deleteUser(userItem: User) {
        ...
        self.managedObjectContext.deleteObject(userItem)
        self.postUpdateNotification()
    }
